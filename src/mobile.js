...
<body>
  ...
  <script>
    ...
    function submitQuickAdd() {
      ...
      if (!text) return;
      fetch('/api/tasks/messy', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: text }) })
        .then(function() { closeQuickAdd(); loadData(); });
    }
    ...
    function loadData() {
      fetch('/api/board')
        ...
        .catch(function(e) {
          document.getElementById('content').innerHTML = '<div class="empty"><div class="empty-icon">&#9888;</div>Failed to load</div>';
        });
    }
    ...
    function render() {
      ...
      if (activeTab === 'projects') {
        ...
        for (var i = 0; i < data.projects.length; i++) {
          ...
          html += '<div class="project-card ' + (p.active ? 'active' : '') + '">';
          html += '<div class="project-header"><div class="badge">' + p.id + '</div>';
          html += '<div class="project-name">' + p.name + '</div>';
          html += '<div class="status-light ' + (p.active ? 'active' : '') + '"></div></div>';
          if (p.status_notes) html += '<div class="project-notes">' + p.status_notes + '</div>';
          html += '</div>';
        }
        html += '</div>';
      }
      else if (activeTab === 'tasks') {
        if (!data.cleanTasks || !data.cleanTasks.length) {
          content.innerHTML = '<div class="empty"><div class="empty-icon">&#10003;</div>No tasks yet</div>';
          return;
        }
        html = '<div class="section"><div class="section-title">Tasks <span class="count">' + data.cleanTasks.length + '</span></div>';
        for (var i = 0; i < data.cleanTasks.length; i++) {
          var t = data.cleanTasks[i];
          html += '<div class="task-item"><div class="badge">' + t.id + '</div>';
          html += '<div class="task-text">' + t.text + '</div>';
          html += '<button class="task-delete" onclick="deleteItem(' + t.id + ')">&times;</button></div>';
        }
        html += '</div>';
      }
      else if (activeTab === 'dump') {
        if (!data.messyTasks || !data.messyTasks.length) {
          content.innerHTML = '<div class="empty"><div class="empty-icon">&#128465;</div>Dump is empty<br><small>Tap + to add</small></div>';
          return;
        }
        html = '<div class="section"><div class="section-title">Dump <span class="count">' + data.messyTasks.length + '</span></div>';
        for (var i = 0; i < data.messyTasks.length; i++) {
          var t = data.messyTasks[i];
          html += '<div class="messy-item"><div class="messy-header"><div class="badge">' + t.id + '</div>';
          html += '<div class="messy-text">' + t.text + '</div>';
          html += '<button class="messy-delete" onclick="deleteItem(' + t.id + ')">&times;</button></div></div>';
        }
        html += '</div>';
      }
      else if (activeTab === 'notepads') {
        if (!data.notepads || !data.notepads.length) {
          content.innerHTML = '<div class="empty"><div class="empty-icon">&#128221;</div>No notepads yet</div>';
          return;
        }
        html = '<div class="section">';
        for (var i = 0; i < data.notepads.length; i++) {
          var n = data.notepads[i];
          var isPinned = data.pinnedNotepads && data.pinnedNotepads.indexOf(n.id) >= 0;
          html += '<div class="notepad-card"><div class="notepad-header"><div class="badge">' + n.id + '</div>';
          html += '<div class="notepad-title">' + n.title + '</div>';
          html += '<button class="notepad-pin" onclick="togglePin(' + n.id + ', ' + isPinned + ')">' + (isPinned ? '&#128204;' : '&#128205;') + '</button></div>';
          if (n.items && n.items.length) {
            for (var j = 0; j < n.items.length; j++) {
              var item = n.items[j];
              html += '<div class="notepad-item"><div class="notepad-check ' + (item.done ? 'done' : '') + '" onclick="toggleCheck(' + n.id + ', ' + item.id + ', ' + item.done + ')">' + (item.done ? '&#10003;' : '') + '</div>';
              html += '<div class="notepad-item-text ' + (item.done ? 'done' : '') + '">' + item.text + '</div></div>';
            }
          } else {
            html += '<div style="color:#9ca3af;font-style:italic;padding:8px 0;">Empty notepad</div>';
          }
          html += '</div>';
        }
        html += '</div>';
        ...
        if (!data.checkins || !data.checkins.length) {
          content.innerHTML = '<div class="empty"><div class="empty-icon">&#128202;</div>No checkins yet</div>';
          return;
        }
        html = '<div class="section"><div class="section-title">Progress <span class="count">' + data.checkins.length + '</span></div>';
        for (var i = 0; i < data.checkins.length; i++) {
          var c = data.checkins[i];
          html += '<div class="checkin-card"><div class="checkin-header">';
          html += '<div class="badge small">' + c.id + '</div>';
          html += '<div class="checkin-time">' + timeAgo(c.created_at) + '</div>';
          if (c.project_name) html += '<span class="checkin-project">' + c.project_name + '</span>';
          html += '</div>';
          html += '<div class="checkin-summary">' + c.summary + '</div></div>';
        }
        html += '</div>';
      }
    ...
    function deleteItem(id) {
      ...
      fetch('/api/item/' + id, { method: 'DELETE' }).then(function() { loadData(); });
    }

    function togglePin(id, isPinned) {
      fetch('/api/notepads/' + id + '/' + (isPinned ? 'unpin' : 'pin'), { method: 'POST' }).then(function() { loadData(); });
    }

    function toggleCheck(notepadId, itemId, isDone) {
      fetch('/api/notepads/' + notepadId + '/items/' + itemId, {
        method: 'PATCH',
